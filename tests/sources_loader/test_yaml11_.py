...
class TestYaml11Loader:
    ...
    def test_comprehensive_type_conversion(self, all_types_yaml11_file: Path):
        ...
        result = load(LoadMetadata(file_=all_types_yaml11_file, loader=Yaml11Loader), AllPythonTypesCompact)
    ...
    def test_yaml_with_prefix(self, prefixed_yaml_file: Path):
        ...
        result = load(
            LoadMetadata(file_=prefixed_yaml_file, loader=Yaml11Loader, prefix="app"),
            PrefixedConfig,
        )
    ...
    def test_yaml_env_var_substitution(self, yaml_config_with_env_vars_file: Path, monkeypatch):
        ...
        result = load(
            LoadMetadata(file_=yaml_config_with_env_vars_file, loader=Yaml11Loader),
            EnvConfig,
        )
    ...
    def test_yaml_env_var_partial_substitution(self, tmp_path: Path, monkeypatch):
        ...
        result = load(LoadMetadata(file_=yaml_file, loader=Yaml11Loader), Config)
    ...
    def test_bool_in_int_field_raises_error(self, tmp_path: Path):
        ...
        with pytest.raises(DatureConfigError) as exc_info:
            load(LoadMetadata(file_=yaml_file, loader=Yaml11Loader), Config)
