...
| `DATURE_ERROR_DISPLAY__MAX_VISIBLE_LINES` | [ErrorDisplayConfig](#errordisplayconfig) | `max_visible_lines` | Max lines shown in one error block for source file previews |
| `DATURE_ERROR_DISPLAY__MAX_ERRORS` | [ErrorDisplayConfig](#errordisplayconfig) | `max_errors` | Max number of error blocks shown for any dature error group — validation, merge conflicts, field groups, env-var expansion, cross-source references, and [strict mode](../advanced/strict-mode.md) unknown keys alike |
...
| `DATURE_LOADING__STRICT` | [LoadingConfig](#loadingconfig) | `strict` | Unknown-key detection: `off` (default), `warn`, or `error`. See [Strict Mode](../advanced/strict-mode.md) |
